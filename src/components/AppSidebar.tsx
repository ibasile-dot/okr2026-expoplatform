...
const updatesItem = { suffix: "/updates", label: "Updates", icon: Rocket };
            ...
            <div key={okr.id}>
              ...
              {isExpanded && (
                <div className="ml-6 pl-3 border-l border-sidebar-border/50 space-y-0.5 mt-0.5">
                  {[...defaultSubItems, ...(okr.id === 4 ? okr4ExtraItems : []), updatesItem].map((sub) => (
                    <RouterNavLink
                      key={sub.suffix}
      ...
      <div className="px-5 py-4 border-t border-sidebar-border">
        {profile && (
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              onClick={signOut}
              className="p-1.5 rounded hover:bg-sidebar-accent/50 text-sidebar-foreground hover:text-primary-foreground transition-colors shrink-0"
              title="Sign out"
            >
        ...
        <img src={epLogo} alt="ExpoPlatform" className="h-8 brightness-0 invert" />
      </div>
    </aside>
  );
};
